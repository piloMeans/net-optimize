...
static netdev_tx_t veth_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	while(tmp && tmp->status && tmp->netdev){
		if(ether_addr_equal_64bits(eth->h_dest, tmp->netdev->dev_addr))
			goto diff;
#ifdef MYDEBUG
	//	printk(KERN_INFO "pkt redirect to dev %s\n",tmp->netdev->name);
#endif
...
out:
#ifdef MYDEBUG
	//	printk(KERN_INFO "pkt not redirect to anydev \n");
#endif
	;
#endif
#endif
...
static __init int veth_init(void)
{
	...
	for(i=0;i<VF_ARRAY_MAX;i++){
		vf_array[i].status=0;
		vf_array[i].netdev=NULL;
		vf_array[i].next=NULL;
		vf_array[i].rnext=NULL;
		vf_array[i].rprev=NULL;
	}
